...
case "$MODE" in
    gui)
        ...
        echo "  ./runapp.sh                                                                  # 일반 실행"
        echo "  ./runapp.sh --ext-folder /isaac-sim/mcp-ext --enable isaac.sim.mcp_extension # MCP 활성화 (Claude/Cursor 연동)"
        echo ""
        echo "MCP 동작 확인 (호스트 별도 터미널):"
        echo "  ss -tnlp | grep 8766    # LISTEN 잡히면 OK"
        echo ""
