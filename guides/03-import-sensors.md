...
- `frameId`: `CameraLeft` (TF에 등록된 카메라 optical frame 이름. Step 4 참고)
...
|-------|---------|-----------|
| Left | `base_link` | `/laser_scan_left` |
| Right | `base_link` | `/laser_scan_right` |

> **frameId 참고**: TF 트리에 실제 존재하는 frame 이름을 사용해야 합니다.
> LiDAR prim은 TF에 자동 포함되지 않으므로 parent link인 `base_link`를 사용합니다.
> 상세한 설명은 [Step 4: Publish TF Tree](04-publish-tf.md)를 참고하세요.
...
isaacsim
```
**이전**: [Step 2: Import URDF](02-import-urdf.md)
**다음**: [Step 4: Publish TF Tree](04-publish-tf.md)
